...
function usePositiveOffset(offset) {
    return framer_motion_1.useTransform(offset, v => -v);
}
exports.usePositiveOffset = usePositiveOffset;
function useSpeed(positiveOffset, ...rangeSpeedPairs) {
    return useParallax(positiveOffset, ...rangeSpeedPairs.map(v => (typeof v === "number" ? speed(v) : v)));
}
exports.useSpeed = useSpeed;
function useSticky(positiveOffset, ...ranges) {
    return useSpeed(positiveOffset, ...shallowFlatten(ranges.map(range => [range, -1])));
}
...
function useParallax(positiveOffset, ...rangeFunPairs) {
    const processedRangeFunPairs = preprocessRangeFun(rangeFunPairs);
    const getRange = index => processedRangeFunPairs[index] && processedRangeFunPairs[index][0];
    const getFun = index => processedRangeFunPairs[index] && processedRangeFunPairs[index][1];
    return framer_motion_1.useTransform(positiveOffset, v => {
        let lastV = 0;
...
function useTrigger(positiveOffset, range, actionFun) {
    React.useEffect(() => {
        let lastV = 0, lastDirection = 0;
